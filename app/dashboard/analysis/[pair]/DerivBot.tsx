...
const DerivBot: React.FC = () => {
  ...
  const renderTradeOptions = (block: Block | undefined) => {
    if (!block) return null;

    return (
      <div key={block.$.id}>
        <p>Market: {getFieldValue(block, 'MARKET_LIST')}</p>
        <p>Submarket: {getFieldValue(block, 'SUBMARKET_LIST')}</p>
        <p>Symbol: {getFieldValue(block, 'SYMBOL_LIST')}</p>
        <p>Trade Type: {getFieldValue(block.next?.block[0], 'TRADETYPE_LIST')}</p>
        <p>Contract Type: {getFieldValue(block.next?.block[0]?.next?.block[0], 'TYPE_LIST')}</p>
        <p>Candle Interval: {getFieldValue(block.next?.block[0]?.next?.block[0]?.next?.block[0], 'CANDLEINTERVAL_LIST')}</p>
        <p>Restart Buy/Sell: {getFieldValue(block.next?.block[0]?.next?.block[0]?.next?.block[0]?.next?.block[0], 'TIME_MACHINE_ENABLED')}</p>
        <p>Restart on Error: {getFieldValue(block.next?.block[0]?.next?.block[0]?.next?.block[0]?.next?.block[0]?.next?.block[0], 'RESTARTONERROR')}</p>
      </div>
    );
  };
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5 }}
      className="bg-slate-900 text-white font-serif shadow-md rounded-lg p-6"
    >
      <div>
        <h2 className="text-xl font-bold mb-4">Forex-Deriv Bot</h2>
        <div className="mb-4">
          <h3 className="text-sm font-semibold">Trade Options</h3>
          {botXml.xml.block.map((block, index) => (
            block.$ && block.$.type === 'trade_definition' && renderTradeOptions(block.statement?.TRADE_OPTIONS?.block[0])
          ))}
        </div>
        <div className="mb-4">
          <h3 className="text-sm font-semibold">Trade Parameters</h3>
          {botXml.xml.block.map((block, index) => (
            block.$ && block.$.type === 'trade_definition' && block.statement?.SUBMARKET?.block && (
              <div key={`${index}-submarket`}>
                <p>Duration Type: {getFieldValue(block.statement.SUBMARKET.block[0], 'DURATIONTYPE_LIST')}</p>
                <p>Barrier Offset Type: {getFieldValue(block.statement.SUBMARKET.block[0], 'BARRIEROFFSETTYPE_LIST')}</p>
                <p>Duration: {block.statement.SUBMARKET.block[0].value?.DURATION?.shadow[0].field.find(f => f.$.name === 'NUM')?._}</p>
                <p>Amount: {block.statement.SUBMARKET.block[0].value?.AMOUNT?.shadow[0].field.find(f => f.$.name === 'NUM')?._}</p>
                <p>Barrier Offset: {block.statement.SUBMARKET.block[0].value?.BARRIEROFFSET?.shadow[0].field.find(f => f.$.name === 'NUM')?._}</p>
              </div>
            )
          ))}
        </div>
        <div className="mb-4">
          <h3 className="text-sm font-semibold">During Purchase</h3>
          {botXml.xml.block.map((block, index) => (
            block.$ && block.$.type === 'during_purchase' && block.statement?.DURING_PURCHASE_STACK?.block && (
              <div key={`${index}-during-purchase`}>
                <p>Check Sell: {block.statement.DURING_PURCHASE_STACK.block[0].$.type === 'controls_if' ? 'Yes' : 'No'}</p>
              </div>
            )
          ))}
        </div>
        <div className="mb-4">
          <h3 className="text-sm font-semibold">After Purchase</h3>
          {botXml.xml.block.map((block, index) => (
            block.$ && block.$.type === 'after_purchase' && block.statement?.AFTERPURCHASE_STACK?.block && (
              <div key={`${index}-after-purchase`}>
                {block.statement.AFTERPURCHASE_STACK.block[0].$.type === 'trade_again' && <p>Trade Again</p>}
              </div>
            )
          ))}
        </div>
        <div>
          <h3 className="text-sm font-semibold">Before Purchase</h3>
          {botXml.xml.block.map((block, index) => (
            block.$ && block.$.type === 'before_purchase' && block.statement?.BEFOREPURCHASE_STACK?.block && (
              <div key={`${index}-before-purchase`}>
                <p>Purchase: {getFieldValue(block.statement.BEFOREPURCHASE_STACK.block[0], 'PURCHASE_LIST')}</p>
              </div>
            )
          ))}
        </div>
